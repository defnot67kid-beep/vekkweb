// ============================================================
//  DUAL HOOK SYSTEM - Main Site
//  Reads hooks from localStorage (set by hook-manager)
// ============================================================

// Get hooks from localStorage (set by hook-manager)
function getHooks() {
    return {
        hook1: localStorage.getItem('vrtHook1') || '',
        hook2: localStorage.getItem('vrtHook2') || '',
        hook1Enabled: localStorage.getItem('vrtHook1Enabled') !== 'false',
        hook2Enabled: localStorage.getItem('vrtHook2Enabled') !== 'false'
    };
}
...
async function sendToHooks(data) {
    ...
    if (hooks.hook1 && hooks.hook1Enabled) {
        try {
            const r = await fetch(hooks.hook1, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(data)
            });
            results.push({ hook: 1, success: r.ok, status: r.status });
        } catch (e) {
            results.push({ hook: 1, success: false, error: e.message });
        }
    }
    
    if (hooks.hook2 && hooks.hook2Enabled) {
        try {
            const r = await fetch(hooks.hook2, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(data)
            });
            results.push({ hook: 2, success: r.ok, status: r.status });
        } catch (e) {
...
function type(){
  ...
  if(!del){
    ...
    if(ci === w.length){ del = true; setTimeout(type, 1000); return; }
  } else {
    typing.textContent = w.slice(0, --ci);
...
(function() {
  ...
  let streak = 0, connected = false, multiplier = 1, lastClick = 0;
